/**
 * SVG Icon utilities
 *
 * Pre-creates SVG elements to avoid repeated innerHTML parsing.
 * Use getIcon() to get a clone of a cached icon.
 */

/** Icon names */
export type IconName =
  | 'play'
  | 'pause'
  | 'volume-high'
  | 'volume-medium'
  | 'volume-muted';

/** SVG icon definitions */
const ICON_PATHS: Record<IconName, { viewBox: string; path: string; size: number }> = {
  play: {
    viewBox: '0 0 24 24',
    path: 'M8 5v14l11-7z',
    size: 24,
  },
  pause: {
    viewBox: '0 0 24 24',
    path: 'M6 19h4V5H6v14zm8-14v14h4V5h-4z',
    size: 24,
  },
  'volume-high': {
    viewBox: '0 0 24 24',
    path: 'M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z',
    size: 20,
  },
  'volume-medium': {
    viewBox: '0 0 24 24',
    path: 'M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z',
    size: 20,
  },
  'volume-muted': {
    viewBox: '0 0 24 24',
    path: 'M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z',
    size: 20,
  },
};

/** Cache of pre-created SVG elements */
const iconCache = new Map<IconName, SVGSVGElement>();

/**
 * Create an SVG element for the given icon
 */
function createSvgElement(name: IconName): SVGSVGElement {
  const def = ICON_PATHS[name];
  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', def.viewBox);
  svg.setAttribute('width', String(def.size));
  svg.setAttribute('height', String(def.size));

  const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
  path.setAttribute('fill', 'currentColor');
  path.setAttribute('d', def.path);
  svg.appendChild(path);

  return svg;
}

/**
 * Get a clone of a cached SVG icon
 *
 * @param name Icon name
 * @returns Cloned SVG element
 */
export function getIcon(name: IconName): SVGSVGElement {
  let cached = iconCache.get(name);
  if (!cached) {
    cached = createSvgElement(name);
    iconCache.set(name, cached);
  }
  return cached.cloneNode(true) as SVGSVGElement;
}

/**
 * Set the icon content of a button element
 * Replaces all children with the icon
 *
 * @param button Button element to update
 * @param name Icon name
 */
export function setButtonIcon(button: HTMLElement, name: IconName): void {
  // Clear existing content
  button.textContent = '';
  button.appendChild(getIcon(name));
}
